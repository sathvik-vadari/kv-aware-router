...
def test_match_rounds_down_to_block_boundary():
    ...
    tree = PrefixTree(prefix_match_unit=16)
...
def test_eviction_is_lru_across_independent_prefixes():
    tree = PrefixTree(prefix_match_unit=16, capacity_tokens={"a": 100})
...
def test_eviction_takes_the_tail_and_degrades_gracefully():
    ...
    tree = PrefixTree(prefix_match_unit=16, capacity_tokens={"a": 250})
    tree.insert(SEQ[:100], "a", now=1.0)
...
def test_match_unit_is_not_the_physical_block_size():
    # The rounding granularity is the engine's prefix match unit, which vLLM
    # allows to be far finer than how it physically stores blocks. Same cached
    # prefix, three engines, three different amounts of usable reuse.
    for unit, expected in [(16, 48), (32, 32), (64, 0)]:
        tree = PrefixTree(prefix_match_unit=unit)
        tree.insert(SEQ[:50], "a", now=1.0)
        assert tree.match(SEQ[:50]) == ({"a": expected} if expected else {})
